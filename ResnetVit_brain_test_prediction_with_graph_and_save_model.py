...
def trainAttentionModel(model, criterion, optimizer, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train']:#, 'val'
            ...
            accuracy_plt.append(epoch_acc.item())
...
##################
# def getVitClassify():
# from  Resnet_VIT_Updated_New_F1.pth import VIT
...
train = DataLoader(train_set, batch_size=2, shuffle=True, drop_last=True)
...
torch.save(model,"Resnet_VIT_Updated_New_F1_for_graph_patch_1_10_epoch.pth")
# model = torch.load("Resnet_VIT_Updated_New_F1.pth")
...
plt.plot(epoch_plt, accuracy_plt)
...
plt1.plot(epoch_plt, loss_plt)
plt1.title('model loss')
plt1.ylabel('loss')
plt1.xlabel('epoch')
plt.legend(['train-loss'], loc='upper left')
plt1.show()
# plt1.savefig("loss.jpg")